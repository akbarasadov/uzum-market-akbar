...
let res1 = await fetch(`http://localhost:8080/products`);
let products = await res1.json();

let type = localStorage.getItem("type_products")
...
if (type === "popular_products") {
    ...
    popular = products.filter((product) => product.rating >= 5);
} else {
    h1.innerHTML = "По карте Uzum"
    popular = products
}

let productPlace = document.querySelector(".grid");
let pices = document.querySelector(".pices");


pices.textContent = `В подборке ${popular.length} товар из категорий`



reload(popular, Products, productPlace);
